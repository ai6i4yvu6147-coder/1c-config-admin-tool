...
- **Полный E2E** (export → apply → rebuild → index): Hub **H6** после P0 CLI `rebuild-index` у config-mcp.
...
- **Сейчас:** apply + UI hint; CLI `rebuild-index` — P0 config-mcp.
...
| `infobaseId` в fragment | `infobases.id` | `ConfigurationExport.id` |
| Линковка | Ручная на экране MCP | Auto-sync по Client |
| Rebuild | UI hint | Hub orchestration после CLI P0 |
...
| H6 | Orchestration `rebuild-index` | config-mcp P0 CLI |
...
| Приоритет | Задача |
|-----------|--------|
| **P0** | CLI `rebuild-index`, `rebuild-all`, `reconcile-markers` |
| **P1** | `status --json` readiness per database |
