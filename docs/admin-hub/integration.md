...
| Hub MCP `configadmin mcp serve` (passive context) | **готово** (2026-07-02, D-H5) |

| data-mcp D-H1–D-H5 (settings, sync, context) | **готово** (2026-07-02) |
...
**Статус:** agreed + Sub ack 2026-07-01; v1.0.5 ack **2026-07-02** (`20260702T180000`). **Канон:** [`registry-mapping-data-mcp.md`](registry-mapping-data-mcp.md), protocol [`protocol-v1.0.4-addendum.md`](../group/shared/protocol-v1.0.4-addendum.md), [`protocol-v1.0.5-addendum.md`](../group/shared/protocol-v1.0.5-addendum.md).

Кратко: S3 keys только в `credentials.sealed.json`; Hub хранит D-MCP password (managed); агент — refs через `resolve_infobase_context` (`projectFilter`, `extensionFilter`, `dataMcp.databaseId`, `credentialsState`; `configadmin mcp serve`), без credentials; runtime unlock — D-MCP `unlock_credentials` (v1.0.6). **Portable writes (v1.0.5):** только через D-MCP CLI (`apply-secrets` + `apply-registry` + `validate-config`); Hub orchestration — `DataMcpSyncService` (D-H4 **done**). **D-MCP UI:** «Сохранить в Hub» (SQLite only) отдельно от «Синхронизировать portable» (CLI).
...
| Hub MCP (passive) | `InfobaseContextService`, `configadmin mcp serve` |
...
- **Phase 3 (Hub):** H6 rebuild orchestration — **done**; help-mcp links — backlog; **data-mcp** — Sub P1 + v1.0.5 + v1.0.6 ack **done**, D-H1–D-H5 **done** (2026-07-02)
